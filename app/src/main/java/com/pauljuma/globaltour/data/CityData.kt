package com.pauljuma.globaltour.data
...
object VocationSpot {
    ...
    var cityList: ArrayList<CityData>? = null
        get() {
            if (field != null) {
                return field
            }
            ...
            for (i in citiesImages.indices) {
                val imageId = citiesImages[i]
                val cityName = cityName[i]
                val cityList = CityData(imageId, cityName, false)
                field!!.add(cityList)
            }
            return field
        }

    // Adding a list of the favorite city.
    var favoriteCities: MutableList<CityData> = mutableListOf()
